...
function CampaignStatusBadge({ status }: { status: Campaign['status'] }) {
  const cfg: Record<Campaign['status'], { bg: string; text: string; border: string; label: string; pulse?: boolean }> = {
    draft:     { bg: 'bg-gray-500/15', text: 'text-white/40', border: 'border-gray-500/30', label: 'Draft' },
...
function TypeBadge({ type }: { type: Campaign['type'] }) {
  ...
  return (
    <div className={`flex items-center gap-1 ${c.color}`}>
      ...
      <span className="text-[10px] uppercase tracking-[0.1em]">{c.label}</span>
    </div>
  );
}
  ...
  return (
    <div className="w-full">
      <div className="flex justify-between text-[10px] mb-1">
        <span className="text-white/25">{current} / {total} calls</span>
    ...
    <section className="space-y-5 animate-fadeInUp">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-white/25">{totalCampaigns} campaigns, {activeCampaigns} active</p>
        </div>
        <Link
          href="/campaigns/new"
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
        {[
          ...
          <div key={stat.label} className="stat-mini p-3">
            <p className="text-[10px] text-white/25 uppercase tracking-[0.1em]">{stat.label}</p>
      ...
      {loading ? (
        ...
        <div className="glass-panel p-12 text-center">
          <svg className="w-12 h-12 mx-auto mb-4 text-white/15" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M11 5.882V19.24a1.76 1.76 0 01-3.417.592l-2.147-6.15M18 13a3 3 0 100-6M5.436 13.683A4.001 4.001 0 017 6h1.832c4.1 0 7.625-1.234 9.168-3v14c-1.543-1.766-5.067-3-9.168-3H7a3.988 3.988 0 01-1.564-.317z" />
          </svg>
          <h3 className="text-white/90 font-medium tracking-tight mb-1">No Campaigns Yet</h3>
          <p className="text-sm text-white/25 mb-4">Create your first campaign to start making calls.</p>
          <Link
            href="/campaigns/new"
              ...
              >
                <div className="p-5">
                  ...
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-3 mb-1">
                        <Link href={`/campaigns/${campaign.id}`} className="text-lg font-bold tracking-tight text-white/90 hover:text-blue-400 transition-colors">
                          {campaign.name}
                        </Link>
                        <CampaignStatusBadge status={campaign.status} />
                        <TypeBadge type={campaign.type} />
                      </div>
                      <p className="text-sm text-white/25 line-clamp-1">{campaign.description}</p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-6 gap-4 mb-4">
                    <div>
                      <p className="text-[10px] text-white/15 uppercase tracking-[0.1em]">Script</p>
                      <p className="text-sm text-white/60 truncate">{campaign.script_name}</p>
                    </div>
                    <div>
                      <p className="text-[10px] text-white/15 uppercase tracking-[0.1em]">Leads</p>
                      <p className="text-sm text-white/90 font-mono">{campaign.lead_count}</p>
                    </div>
                    <div>
                      <p className="text-[10px] text-white/15 uppercase tracking-[0.1em]">Calls Made</p>
                      <p className="text-sm text-white/90 font-mono">{campaign.calls_made} / {campaign.lead_count}</p>
                    </div>
                    <div>
                      <p className="text-[10px] text-white/15 uppercase tracking-[0.1em]">Appointments</p>
                      <p className="text-sm text-emerald-400 font-mono font-bold tracking-tight">{campaign.appointments_booked}</p>
                    </div>
                    <div>
                      <p className="text-[10px] text-white/15 uppercase tracking-[0.1em]">Conversion</p>
                      <p className="text-sm text-amber-400 font-mono">{campaign.calls_answered > 0 ? `${(campaign.conversion_rate * 100).toFixed(1)}%` : '--'}</p>
                    </div>
                    <div>
                      <p className="text-[10px] text-white/15 uppercase tracking-[0.1em]">Cost</p>
                      <p className="text-sm text-white/60 font-mono">${campaign.total_cost.toFixed(2)}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 pt-3 border-t border-white/[0.04]">
                    ...
                    <Link
                      ...
                      className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-white/5 border border-white/[0.04] text-white/60 text-xs hover:bg-white/10 transition-colors"
                    >
                      <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}>
